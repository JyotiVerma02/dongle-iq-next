...
export default function DongleIQLanding() {
  ...
  return (
    <div
      className={`min-h-screen ${theme.bg} text-white font-sans scroll-smooth`}
    >
      {/* --- HEADER / MENU --- */}
      <nav
        className={`fixed top-0 w-full z-50 p-6 ${theme.bg}/80 backdrop-blur-md border-b ${theme.border}`}
      >
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div
              className={`w-8 h-8 ${theme.accent} rounded-lg flex items-center justify-center`}
            >
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <h1 className="text-6xl md:text-8xl font-black tracking-tighter uppercase leading-[0.85] mb-8">
              Digital <span className="text-white/20 italic">ID</span> <br />{" "}
              Simplified.
            </h1>
            <p className={`${theme.textMuted} text-lg mb-10 max-w-md`}>
              The premium SaaS platform for managing Dongle-based Agent IDs and
              IRCTC Digital Signature Certificates.
            </p>
          ...
          <div
            className={`h-100 ${theme.card} border ${theme.border} rounded-[40px] relative overflow-hidden flex items-center justify-center`}
          >
      ...
      <section id="about" className="py-24 px-6 bg-[#0a0e17]">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-4xl font-black uppercase tracking-tighter mb-6">
            Built for <span className="text-purple-500">Security</span>
          </h2>
          <p className={`${theme.textMuted} text-lg leading-relaxed`}>
            Dongle IQ is a web platform specifically designed for agent ID USB
            registrations. We bridge the gap between complex government
            compliance and user-friendly digital dashboards, ensuring your IRCTC
            DSC workflow is seamless and verified.
          </p>
        </div>
      </section>
      ...
      <section id="contact" className="py-24 px-6">
        <div
          className={`max-w-5xl mx-auto ${theme.card} border ${theme.border} rounded-[40px] p-12 grid grid-cols-1 md:grid-cols-2 gap-12`}
        >
          <div>
            <h2 className="text-4xl font-black uppercase tracking-tighter mb-6">
            ...
            <p className={theme.textMuted + " mb-8"}>
              Need help with your DSC registration? Our Gurugram-based support
              team is here.
            </p>
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-4 text-sm font-bold">
                <MessageSquare size={18} className="text-purple-500" /> Live
                Chat Available
              </div>
            </div>
          </div>
          <form className="space-y-4">
            <input
              ...
              className={`w-full ${theme.bg} border ${theme.border} p-4 rounded-xl text-sm focus:border-purple-500 outline-none`}
            />
            <textarea
            ...
            />
            <button
              className={`${theme.accent} w-full py-4 rounded-xl text-xs font-black uppercase tracking-widest`}
            >
              Send Message
            </button>
          </form>
...
function ServiceCard({ title, desc, icon, theme }: any) {
  return (
    <div
      className={`${theme.card} border ${theme.border} p-10 rounded-4xl hover:border-purple-500/50 transition-all group`}
    >
...
function FaqItem({ q, a, index, active, setActive, theme }: any) {
  ...
  return (
    ...
    >
      ...
      {isOpen && (
        <div
          className={`p-6 pt-0 text-sm ${theme.textMuted} leading-relaxed border-t ${theme.border} bg-white/5`}
        >
          {a}
        </div>
      )}
