        ...
        <div className="bg-[#0f1219]/80 backdrop-blur-2xl border border-white/10 rounded-2xl overflow-hidden shadow-2xl relative">
          ...
          <div className="p-4 border-b border-white/5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white/[0.02]">
            ...
            <div className="flex flex-wrap gap-3">
              <FilterBadge
                ...
                colors={["text-emerald-400", "text-red-400"]}
              />
              <FilterBadge
                label="Wallet Status"
                options={["Pending", "Active"]}
                colors={["text-blue-400", "text-emerald-400"]}
              />
                  ...
                  filteredApplications.map((app) => (
                    ...
                    >
                      <td className="p-4 pl-6 text-white/40 font-mono text-xs">
                        #{app.id?.toString().slice(-6) || "000000"}
                      </td>
                      <td className="p-4">
                        <div className="flex items-center gap-3">
          ...
          <div className="px-6 py-4 border-t border-white/5 bg-white/[0.01] flex justify-between items-center text-xs text-white/40">
            ...
            <div className="flex gap-2">
              <button
                className="px-3 py-1 bg-white/5 border border-white/10 rounded hover:bg-white/10 transition disabled:opacity-50"
                disabled
              >
                Previous
              </button>
          ...
          >
            ...
            >
              ...
              <div className="mt-6 flex justify-end gap-3">
                ...
                <button className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold rounded-lg transition shadow-lg shadow-blue-600/20">
                  Edit Record
                </button>
              </div>
            </div>
...
function Card({ title, value, color, icon }) {
  ...
  return (
    <div
      className={`relative p-5 rounded-2xl border ${theme} backdrop-blur-md overflow-hidden group transition-all duration-300 hover:-translate-y-1 hover:shadow-xl`}
    >
    ...
    <div className="flex items-center bg-[#0a0c10] border border-white/10 rounded-xl overflow-hidden h-10 shadow-sm">
      ...
      <div className="flex px-3 gap-3">
        {options.map((opt, i) => (
          <label
            key={i}
            className="flex items-center gap-1.5 cursor-pointer hover:opacity-100 opacity-70 transition"
          >
            <input
              type="checkbox"
              className="w-3 h-3 rounded bg-white/10 border-white/20 checked:bg-blue-500 checked:border-blue-500 accent-blue-500 appearance-none"
              defaultChecked
            />
...
function StatusBadge({ status }) {
  const styles = {
    ...
    Approved:
      "bg-emerald-400/10 text-emerald-400 border-emerald-400/20 shadow-[0_0_10px_rgba(52,211,153,0.1)]",
    Rejected:
      "bg-rose-400/10 text-rose-400 border-rose-400/20 shadow-[0_0_10px_rgba(211,113,133,0.1)]",
  };

  return (
    <span
      className={`inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold border uppercase tracking-wide ${styles[status] || "bg-white/10 text-white"}`}
    >
      {status === "Approved" && (
        <span className="mr-1.5 w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse"></span>
      )}
      {status}
    </span>
  );
