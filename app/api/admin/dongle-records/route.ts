...
export async function GET() {
  try {
    ...
    const users = await User.find({ role: { $ne: "admin" } })
      .select(
        "_id name email number pan ekycId bpCode certType certificateClass tokenType validity status addressProof idProof photo internalRemarks createdAt updatedAt"
      )
      .sort({ updatedAt: -1 })
      .lean();
