...
export default function DSCRegistrationForm() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-4" style={{ fontFamily: "'Plus Jakarta Sans', 'Segoe UI', sans-serif" }}>
        ...
        <div className="bg-[#f8fafc] p-10 border-t border-gray-100">
          <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
              ...
              {[
                { label: "User Type*", key: "userType", options: ["Individual", "Organization", "Foreign Individual"] },
                { label: "Class Type", key: "classType", options: ["Class III"] },
                { label: "Certificate Types*", key: "certType", options: [  "Encryption",  "Signature","Signing & Encryption"] },
                { label: "Validity*", key: "validity", options: ["1 Year", "2 Years", "3 Years"] },
                { label: "Token Type", key: "tokenType", options: ["Not Required", "USB Token"] },
                { label: "Assisted Service", key: "assistedService", options: ["Not Required", "Required"] }
              ].map((item) => (
                <div key={item.key} className="flex flex-col gap-2">
                  <label className="text-[13px] font-black text-gray-900 uppercase tracking-wider">{item.label}</label>
                  <select 
                  ...
                  >
                    {item.key === "certType" || item.key === "validity" ? <option value="">Select Type</option> : null}
                    {item.options.map(opt => <option key={opt} className="text-gray-900">{opt}</option>)}
                  </select>
                </div>
              ))}

              {/* Price Visibility Logic */}
              <div className={`md:col-span-2 flex items-center gap-4 transition-all duration-700 ${isProductSelected ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
                  ...
                  <table className="w-full text-[11px] leading-tight font-bold">
                    <thead>
                      <tr className="bg-gray-900 text-white">
                        <th colSpan="3" className="px-3 py-1.5 text-left italic">Bifurcation (Inc. Tax)</th>
                      </tr>
                      <tr className="bg-gray-50 border-b border-gray-900">
                        <th className="px-3 py-1.5 border-r border-gray-900 text-gray-600">Cert</th>
                        <th className="px-3 py-1.5 border-r border-gray-900 text-gray-600">Token</th>
                        <th className="px-3 py-1.5 text-gray-600">Assisted</th>
                      </tr>
                    </thead>
                    <tbody className="text-gray-900 text-center text-sm">
                      <tr>
                        <td className="px-3 py-2 border-r border-gray-900">₹{pricing.certificate}</td>
                        <td className="px-3 py-2 border-r border-gray-900">₹{pricing.token}</td>
            ...
            <div className="mt-12 pt-8 border-t border-gray-100 flex flex-col md:flex-row items-center justify-between gap-8">
              <div className="flex items-center gap-6">
                <span className="text-sm font-black text-gray-900 uppercase">eKYC Method:</span>
                  ...
                  {["PAN", "Aadhaar"].map(type => (
                    <label key={type} className="flex items-center gap-2.5 cursor-pointer group">
                      <input 
                        ...
                        onChange={() => setFormData({...formData, ekycType: type})} 
                        className="w-5 h-5 accent-blue-600"
                      />
                      <span className="text-sm font-bold text-gray-800 group-hover:text-blue-600">{type}</span>
                    </label>
                  ))}
                </div>
              </div>
              
              <button className="w-full md:w-auto bg-blue-600 hover:bg-blue-700 text-white px-12 py-4 rounded-2xl font-black text-sm uppercase tracking-widest shadow-xl shadow-blue-200 transition-all hover:-translate-y-1 active:scale-95">
                Proceed (Next)
              </button>
            </div>
          </div>
